...
通用日志不可关闭，包含启动开始、Unreal 初始化、每个功能模块的加载结果、模块卸载和关键错误。REST API 请求详细记录仍单独写入 `Logs/RESTAPI/`。每个请求使用“可读摘要 + 缩进 JSON + 分隔线”的日志块：摘要包含方法、路径、HTTP 状态、来源 IP、指令和令牌名称；缩进 JSON 保留完整的请求、响应和鉴权字段，不会因为格式化而丢失信息。
